defmodule Day14P2 do
	...
	def main(mapset) do
		...
		drop_stones(mapset, max_y+2) - size
	end

	def drop_stones(mapset, max_y) do
		result = move(max_y, mapset)
		cond do
			MapSet.member?(result, {500, 0}) -> MapSet.size(result)
			true -> drop_stones(result, max_y)
		end
	end

	def move(max_y, mapset, pos \\ {500, 0})
	def move(max_y, mapset, {x,y}) when y + 1 < max_y do
		below = {x, y+1}
		left = {x-1, y+1}
		right = {x+1, y+1}
		cond do
	...
	def move(max_y, mapset, {x, y}) do MapSet.put(mapset, {x, y}) end
